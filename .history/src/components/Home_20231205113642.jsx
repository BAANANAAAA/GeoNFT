...
function NFTImage({ tokenId, getCount, userCID }) {
  ...
  const mintToken = async () => {
    ...
    try {
  mintTimeoutRef.current = new Promise((resolve, reject) => {
    const timeoutId = setTimeout(async () => {
      ...
      try {
        ...
        reject(mintError); // Reject the promise on error
      }
    }, 2000);

    // Attach the timeoutId to the promise so it can be cleared if needed
    mintTimeoutRef.current = { promise: timeoutId, reject };
  });
} catch (timeoutError) {
